...
# Suppress Apache Beam warnings
logging.getLogger('apache_beam.transforms.core').setLevel(logging.ERROR)
logging.getLogger('apache_beam.runners.common').setLevel(logging.ERROR)
...
def extract_and_upload():
    """
    Download from Google Drive → upload to GCS.
...
def build_dim_customer(rows):
    return (
        ...
        | "Drop null customers" >> beam.Filter(lambda c: c is not None)
        | "Distinct customers" >> beam.Distinct()
        | "Enumerate customers" >> beam.ParDo(AddSurrogateKey(), field_name="customer_id", sk_name="customer_sk")
    )
...
def build_dim_vehicle(rows):
    return (
        ...
        | "Enumerate vehicles" >> beam.ParDo(AddSurrogateKey(), field_name="vehicle_type", sk_name="vehicle_sk")
    )


def build_dim_location(rows):
    pickups = rows | "Extract pickup locations" >> beam.Map(lambda r: r.get("pickup_location"))
    drops   = rows | "Extract drop locations"   >> beam.Map(lambda r: r.get("drop_location"))
    ...
    return (
        ...
        | "Enumerate locations" >> beam.ParDo(AddSurrogateKey(), field_name="location_name", sk_name="location_sk")
    )
...
def build_dim_payment(rows):
    return (
        ...
        | "Enumerate payments" >> beam.ParDo(AddSurrogateKey(), field_name="payment_method", sk_name="payment_sk")
    )
...
def build_dim_reason(rows):
    ...
    return (
        ...
        | "Enumerate reasons" >> beam.ParDo(AddSurrogateKey(), field_name="reason_text", sk_name="reason_sk")
...
def build_fact_booking(rows, dim_customer, dim_vehicle, dim_location, dim_payment, dim_reason):
    customer_map = beam.pvalue.AsDict(dim_customer | "CustomerMap" >> beam.Map(lambda d: (d["customer_id"], d["customer_sk"])))
    vehicle_map  = beam.pvalue.AsDict(dim_vehicle  | "VehicleMap"  >> beam.Map(lambda d: (d["vehicle_type"], d["vehicle_sk"])))
    location_map = beam.pvalue.AsDict(dim_location | "LocationMap" >> beam.Map(lambda d: (d["location_name"], d["location_sk"])))
    payment_map  = beam.pvalue.AsDict(dim_payment  | "PaymentMap"  >> beam.Map(lambda d: (d["payment_method"], d["payment_sk"])))
    reason_map   = beam.pvalue.AsDict(dim_reason   | "ReasonMap"   >> beam.Map(lambda d: (d["reason_text"], d["reason_sk"])))

    def enrich(row, cmap, vmap, lmap, pmap, rmap):
        customer_cancel = row.get("reason_for_cancelling_by_customer")
        driver_cancel   = row.get("driver_cancellation_reason")
        incomplete      = row.get("incomplete_rides_reason")

        return {
            "booking_id": row.get("booking_id"),
            "date_id": int(row["date_id"]) if row.get("date_id") else None,
            ...
            "incomplete_reason_sk": rmap.get(incomplete) if incomplete else None,
            "booking_status": row.get("booking_status"),
            "avg_vtat": float(row["avg_vtat"]) if row.get("avg_vtat") else None,
            "avg_ctat": float(row["avg_ctat"]) if row.get("avg_ctat") else None,
            "booking_value": float(row["booking_value"]) if row.get("booking_value") else None,
            "ride_distance": float(row["ride_distance"]) if row.get("ride_distance") else None,
            "driver_ratings": float(row["driver_ratings"]) if row.get("driver_ratings") else None,
            "customer_rating": float(row["customer_rating"]) if row.get("customer_rating") else None,
        }

    return rows | "BuildFactBooking" >> beam.Map(enrich, customer_map, vehicle_map, location_map, payment_map, reason_map)
...
def run_pipeline(input_path, output_prefix):
    ...
    with beam.Pipeline(options=PipelineOptions()) as p:
       ...
       # ------------------------
        # Step 2: Write cleaned file
        # ------------------------
        print("Step 2: Writing cleaned data...")
        (
            rows
            | "CleanedToCSV" >> beam.Map(lambda r: ",".join([str(r.get(k,"")) for k in r.keys()]))
        ...
        # ------------------------
        # Step 3: Build dims from cleaned rows
        # ------------------------
        ...
        dim_reason   = build_dim_reason(rows)
         ...
         # ------------------------
        # Step 4: Build fact from cleaned rows + dim maps
        # ------------------------
        print("Step 4: Building fact table...")
        fact_booking = build_fact_booking(rows, dim_customer, dim_vehicle, dim_location, dim_payment, dim_reason)

        # ------------------------
        # Step 5: Load dims to BigQuery
        # ------------------------
        print("Step 5: Loading dimensions to BigQuery...")
        ...
        # ------------------------
        # Step 6: Load fact to BigQuery
        # ------------------------
        print("Step 6: Loading fact table to BigQuery...")
        fact_booking | "BQFactBooking" >> beam.io.WriteToBigQuery(
                    ...
                    schema=(
                        ...
                        "incomplete_reason_sk:INTEGER,booking_status:STRING,avg_vtat:FLOAT,avg_ctat:FLOAT,"
                        "booking_value:FLOAT,ride_distance:FLOAT,driver_ratings:FLOAT,customer_rating:FLOAT"
                    ),
                    create_disposition="CREATE_IF_NEEDED",
                    write_disposition="WRITE_TRUNCATE",
                    custom_gcs_temp_location=f"gs://{BUCKET_NAME2}/temp"
        ...
        # ------------------------
        # Step 7: Write tables to GCS bucket
        # ------------------------
        print("Step 7: Writing dimensions to GCS...")
        dim_outputs = {
            ...
            "fact_booking": (["booking_id","date_id","customer_sk","vehicle_sk","pickup_location_sk",
                     ...
                     "ride_distance","driver_ratings","customer_rating"], fact_booking)
        }
