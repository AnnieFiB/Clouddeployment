...
A. create credentials for composer service account(download a json key to alow local connection) and grant roles:
...
C. create buckets
  ...
  bq --location=europe-west2 mk --dataset dedwhetl:uber-raw
  Make a dataset for cleaned/star schema tables===
  bq --location=europe-west2 mk --dataset dedwhetl:uber-transformed

D. create a composer environment and a DAGs folder 
  airflow v2; standard resilience

E. create a bigquery dataset

F. create region for the project
  get project config:  "gcloud config configurations list" /   "gcloud config list"
        NAME: cloudshell-3678
        IS_ACTIVE: True
        ACCOUNT: 
        PROJECT: dedwhetl
        COMPUTE_DEFAULT_ZONE: 
        COMPUTE_DEFAULT_REGION: 
        bthonia@cloudshell:~ (dedwhetl)$ 

  get lists of regions: "gcloud compute regions list"
  set default region:  "gcloud config set compute/region europe-west2"

G. install libraries on local
    python -m pip install "apache-beam[gcp]" google-cloud-storage google-cloud-bigquery apache-airflow-providers-google apache-airflow-providers-apache-beam
...
Option B -  Storage Transfer Service (Drive → GCS), then BigQuery (production‑ready)
  download file to bucket(uber-raw)
    gsutil cp ~/Downloads/rawuber_2025-01-01.csv gs://uber-raw OR
    use extract python script to download raw file from gdrive and upload to bucket



## Step 3 – Transform and normalise
